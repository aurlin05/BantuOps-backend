...
public class DatabasePerformanceTracker {
    ...
    /**
     * Surveillance périodique des performances de base de données (toutes les 5
     * minutes)
     */
    ...
    private void collectPostgreSQLStatistics(Connection connection) throws SQLException {
        ...
        String tableStatsQuery = "SELECT schemaname, tablename, n_tup_ins, n_tup_upd, n_tup_del, " +
        ...
        try (PreparedStatement stmt = connection.prepareStatement(tableStatsQuery);
            ...
            while (rs.next()) {
                ...
                double indexUsageRatio = (seqScans + idxScans) > 0 ? (double) idxScans / (seqScans + idxScans) : 0.0;

                // Calculer le ratio de tuples morts
                double deadTupleRatio = (liveTuples + deadTuples) > 0 ? (double) deadTuples / (liveTuples + deadTuples)
                        : 0.0;
    ...
    private void collectIndexStatistics(Connection connection) throws SQLException {
        String indexStatsQuery = "SELECT schemaname, tablename, indexname, idx_scan, idx_tup_read, idx_tup_fetch " +
    ...
    private void collectActiveQueryStatistics(Connection connection) throws SQLException {
        String activeQueriesQuery = "SELECT count(*) as active_queries, " +
    ...
    private void collectCacheStatistics(Connection connection) throws SQLException {
        String cacheStatsQuery = "SELECT sum(heap_blks_read) as heap_read, " +
        ...
        try (PreparedStatement stmt = connection.prepareStatement(cacheStatsQuery);
            ...
            if (rs.next()) {
                ...
                double heapCacheRatio = (heapRead + heapHit) > 0 ? (double) heapHit / (heapRead + heapHit) : 0.0;
                double indexCacheRatio = (idxRead + idxHit) > 0 ? (double) idxHit / (idxRead + idxHit) : 0.0;
