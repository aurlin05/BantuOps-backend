...
public class AuditReportController {
    ...
    @Operation(summary = "Générer un rapport d'audit sécurisé", description = "Génère un rapport d'audit complet avec chiffrement des données sensibles")
    @ApiResponses(value = {
    ...
    @PostMapping("/export")
    @PreAuthorize("hasAnyRole('ADMIN', 'AUDITOR')")
    @Operation(summary = "Exporter un rapport d'audit", description = "Exporte un rapport d'audit dans le format spécifié avec chiffrement")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Générer un rapport d'audit rapide", description = "Génère un rapport d'audit simplifié pour une période donnée")
    public ResponseEntity<AuditReportResponse> generateQuickAuditReport(
            @Parameter(description = "Date de début (format: yyyy-MM-dd'T'HH:mm:ss)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate,

            @Parameter(description = "Date de fin (format: yyyy-MM-dd'T'HH:mm:ss)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDate,

            @Parameter(description = "Type de rapport") @RequestParam(defaultValue = "COMPREHENSIVE_AUDIT") AuditReportRequest.ReportType reportType,

            @Parameter(description = "Nombre maximum de résultats") @RequestParam(defaultValue = "1000") int maxResults) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'AUDITOR', 'HR')")
    @Operation(summary = "Statistiques d'audit pour le tableau de bord", description = "Récupère les statistiques d'audit récentes pour l'affichage du tableau de bord")
    public ResponseEntity<Map<String, Object>> getDashboardAuditStats(
            @Parameter(description = "Nombre d'heures à inclure dans les statistiques") @RequestParam(defaultValue = "24") int hours) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'AUDITOR')")
    @Operation(summary = "Valider les paramètres d'un rapport", description = "Valide les paramètres d'une demande de rapport d'audit")
    ...
    private java.util.List<String> generateRecommendations(AuditReportRequest request) {
        ...
        if (days > 90) {
            recommendations.add(
                    "Considérez diviser la période en plusieurs rapports plus petits pour de meilleures performances");
        }
